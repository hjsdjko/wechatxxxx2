...
const {
data
} = await detail("wuyinliaoji",id)
this.setData({
payAuth:utils.isAuthFront('wuyinliaoji','支付')
})
...
const resUpdate = await update('wuyinliaoji', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','wuyinliaoji');
...
async reply() {
const detailList = this.data.detailList
const res = await update("wuyinliaoji", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
