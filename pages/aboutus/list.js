...
this.setData({
addAuth: utils.isAuth("aboutus", "新增") || utils.isAuthFront("aboutus", "新增"),
delAuth: utils.isAuth("aboutus", "删除") || utils.isAuthFront("aboutus", "删除"),
editAuth: utils.isAuth("aboutus", "修改") || utils.isAuthFront("aboutus", "修改"),
})
if(options?.userid) {
this.setData({
...
let res = {};
if(this.data.userid) {
res = await page(`aboutus`, searchForm);
} else {
res = await list(`aboutus`, searchForm);
}
...
wx.navigateTo({
url: `/pages/aboutus/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("aboutus",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
