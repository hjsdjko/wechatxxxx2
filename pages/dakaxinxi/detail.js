...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.yonghuxingming,
...
const {
data
} = await detail("dakaxinxi",id)
this.setData({
payAuth:utils.isAuthFront('dakaxinxi','支付')
})
...
const resUpdate = await update('dakaxinxi', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','dakaxinxi');
...
async reply() {
const detailList = this.data.detailList
const res = await update("dakaxinxi", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
