...
this.setData({
addAuth: utils.isAuth("dakaxinxi", "新增") || utils.isAuthFront("dakaxinxi", "新增"),
delAuth: utils.isAuth("dakaxinxi", "删除") || utils.isAuthFront("dakaxinxi", "删除"),
editAuth: utils.isAuth("dakaxinxi", "修改") || utils.isAuthFront("dakaxinxi", "修改"),
})
if(options?.userid) {
this.setData({
...
let res = {};
if(this.data.userid) {
res = await page(`dakaxinxi`, searchForm);
} else {
res = await list(`dakaxinxi`, searchForm);
}
...
wx.navigateTo({
url: `/pages/dakaxinxi/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("dakaxinxi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
