...
tablename: 'shengyinzhumian',
userid:userInfo.id,
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.shengyinmingcheng,
...
const {
data
} = await detail("shengyinzhumian",id)
this.setData({
payAuth:utils.isAuthFront('shengyinzhumian','支付')
})
...
const data = {
            name: this.data.detailList.shengyinmingcheng,
...
const resUpdate = await update('shengyinzhumian', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','shengyinzhumian');
...
async reply() {
const detailList = this.data.detailList
const res = await update("shengyinzhumian", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
