...
const {
data
} = await detail("shengyinleixing",id)
this.setData({
payAuth:utils.isAuthFront('shengyinleixing','支付')
})
...
const resUpdate = await update('shengyinleixing', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','shengyinleixing');
...
async reply() {
const detailList = this.data.detailList
const res = await update("shengyinleixing", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
