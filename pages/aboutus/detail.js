...
const {
data
} = await detail("aboutus",id)
this.setData({
payAuth:utils.isAuthFront('aboutus','支付')
})
...
const resUpdate = await update('aboutus', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','aboutus');
...
async reply() {
const detailList = this.data.detailList
const res = await update("aboutus", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
