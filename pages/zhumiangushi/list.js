...
this.setData({
addAuth: utils.isAuth("zhumiangushi", "新增") || utils.isAuthFront("zhumiangushi", "新增"),
delAuth: utils.isAuth("zhumiangushi", "删除") || utils.isAuthFront("zhumiangushi", "删除"),
editAuth: utils.isAuth("zhumiangushi", "修改") || utils.isAuthFront("zhumiangushi", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            if(this.data.gushimingcheng){
            searchForm['gushimingcheng'] = '%' + this.data.gushimingcheng + '%'
        }
                if(this.data.zuozhe){
            searchForm['zuozhe'] = '%' + this.data.zuozhe + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`zhumiangushi`, searchForm);
} else {
res = await list(`zhumiangushi`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zhumiangushi/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zhumiangushi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
