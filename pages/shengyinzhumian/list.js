...
this.setData({
addAuth: utils.isAuth("shengyinzhumian", "新增") || utils.isAuthFront("shengyinzhumian", "新增"),
delAuth: utils.isAuth("shengyinzhumian", "删除") || utils.isAuthFront("shengyinzhumian", "删除"),
editAuth: utils.isAuth("shengyinzhumian", "修改") || utils.isAuthFront("shengyinzhumian", "修改"),
})
if(options?.userid) {
this.setData({
...
if (currentPageUrl == "/pages/shengyinzhumian/list") {
    wx.setStorageSync("tableName",`shengyinzhumian`)
    this.setData({
...
let res = {};
if(this.data.userid) {
res = await page(`shengyinzhumian`, searchForm);
} else {
res = await list(`shengyinzhumian`, searchForm);
}
...
handleTabClick(e) {
        ...
        const shengyinleixing = e.currentTarget.dataset.shengyinleixing;
        if (shengyinleixing == "全部") {
            const all = this.data.allData.filter(item => item.shengyinleixing != "全部")
            this.setData({
                goodsListData: all
            })
        } else {
            const currentGoodsListData = this.data.allData.filter(item => {
                return item.shengyinleixing == shengyinleixing;
            });
            this.setData({
...
wx.navigateTo({
url: `/pages/shengyinzhumian/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("shengyinzhumian",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/shengyinzhumian/detail`
})
...
const {
    data
}= await page("shengyinzhumian",obj)
...
const {
    data
}= await list("shengyinzhumian",obj)
totalPage=data?.totalPage
...
allData: goodsListData
        ...
        const shengyinleixingobj={
            page:1,
            limit:100
        }
        let    shengyinleixingRes
        //侧边栏
        if(userId){
                shengyinleixingRes     = await list('shengyinleixing',shengyinleixingobj)
        }else{
                shengyinleixingRes     = await list('shengyinleixing',shengyinleixingobj)

        }
                arr.push(...shengyinleixingRes.data.list)
        this.setData({
            silderList:arr
        })
                    },
