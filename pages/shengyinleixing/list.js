...
this.setData({
addAuth: utils.isAuth("shengyinleixing", "新增") || utils.isAuthFront("shengyinleixing", "新增"),
delAuth: utils.isAuth("shengyinleixing", "删除") || utils.isAuthFront("shengyinleixing", "删除"),
editAuth: utils.isAuth("shengyinleixing", "修改") || utils.isAuthFront("shengyinleixing", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            if(this.data.shengyinleixing){
            searchForm['shengyinleixing'] = '%' + this.data.shengyinleixing + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`shengyinleixing`, searchForm);
} else {
res = await list(`shengyinleixing`, searchForm);
}
...
wx.navigateTo({
url: `/pages/shengyinleixing/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("shengyinleixing",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/shengyinleixing/detail`
})
...
const {
    data
}= await page("shengyinleixing",obj)
...
const {
    data
}= await list("shengyinleixing",obj)
totalPage=data?.totalPage
